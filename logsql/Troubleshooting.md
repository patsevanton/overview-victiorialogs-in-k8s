...
Это можно сделать, добавляя ``| count()`` после каждого фильтра или pipe, который может менять количество строк.
...
Замените все `pipes` в запросе на `| count()` и выполните обновлённый запрос, чтобы увидеть общее количество совпадающих логов:

```logsql
_time:5m host:"api-" level:error "database" | count()
```

Пример вывода (получен через `vlogscli`, но можно использовать
`любой поддерживаемый способ выполнения запросов`):
...
Дополнительные рекомендации см. в разделе `Performance Tips`.
...
Фильтр ``_time`` является обязательным: если его нет, VictoriaLogs буквально сканирует **все** логи, хранящиеся в базе данных.
Фильтр `_time` позволяет ограничить объём сканируемых данных заданным временным диапазоном.

Обратите внимание: `Web UI VictoriaLogs` и
`плагин Grafana для VictoriaLogs`
...
Если запрос не содержит `фильтров по потокам логов (log stream filters)`, VictoriaLogs вынужден читать и сканировать все блоки данных в выбранном временном диапазоне.
...
Однако если ваш `log stream` содержит поле `app="nginx"`, а вы напишете фильтр так:
...
`документации по stream-фильтрам`.
...
Pipe ``keep`` оставляет только перечисленные поля логов и удаляет все остальные,
что позволяет получить статистику только по интересующим вас полям.
Включайте все поля, которые используются в `фильтрах` или
`pipes` анализируемого запроса.
...
`Как определить, какие поля логов занимают больше всего места на диске?`.
...
Удалите все `pipes` и оставьте только
`фильтр по времени`, например `_time:5m`.
...
чтобы уменьшить объём данных, обрабатываемых `pipes`.

Например, добавление
`phrase-фильтров`
по постоянным частям строки из шаблона ``extract``
...
* Функции с высокой кардинальностью, такие как ``count_uniq()``, хранят в памяти все уникальные значения. Подумайте, как сократить их количество.
* Большое число групп в ``stats by (...)`` может потреблять много памяти. Отфильтруйте или трансформируйте данные, чтобы уменьшить число групп.
