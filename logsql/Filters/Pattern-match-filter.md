...
VictoriaLogs поддерживает фильтрацию логов по шаблонам с помощью фильтра `pattern_match("шаблон")`. Этот фильтр отбирает записи логов, в которых поле ``_msg`` содержит указанный `"шаблон"`.
...
- `<W>` — соответствует любому `слову` или любой строке в одинарных, двойных кавычках или обратных кавычках.

Такие шаблоны генерируются с помощью конвейера ``collapse_nums``.
