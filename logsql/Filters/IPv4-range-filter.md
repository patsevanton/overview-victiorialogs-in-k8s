...
1. Извлеките IP‑адрес из сообщения с помощью конвейера ``extract``.
2. Примените фильтр `ipv4_range()` к извлечённому полю.

### Полезные советы

- Если нужно найти **сообщения журнала**, содержащие конкретный IPv4‑адрес `X.Y.Z.Q`, используйте запрос `"X.Y.Z.Q"`. Подробнее — в `документации по фильтрам фраз`.
- Если требуется найти сообщения, содержащие **хотя бы один** IPv4‑адрес из списка, используйте запрос `"ip1" OR "ip2" ... OR "ipN"`. Подробнее — в `документации по логическим фильтрам`.
- Чтобы найти записи с полем `ip` в **нескольких диапазонах**, используйте запрос:  
  ...
  ```
  Подробнее — в `документации по логическим фильтрам`.

### Советы по производительности

- Предпочитайте запросы к **чистому IPv4‑полю** вместо извлечения IPv4 из текстового поля с помощью преобразований (`transformations`) во время выполнения запроса.
- Смотрите `другие советы по производительности`.
