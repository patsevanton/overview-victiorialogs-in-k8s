...
Если вы ещё не знакомы с VictoriaLogs, начните с раздела **[ключевых понятий](https://docs.victoriametrics.com/victorialogs/keyconcepts/)**.

Затем ознакомьтесь со следующими документами:

- [Как запустить VictoriaLogs](https://docs.victoriametrics.com/victorialogs/quickstart/).
- [Как загружать данные в VictoriaLogs](https://docs.victoriametrics.com/victorialogs/data-ingestion/).
- [Как выполнять запросы к VictoriaLogs](https://docs.victoriametrics.com/victorialogs/querying/).
...
Для выполнения запросов к VictoriaLogs рекомендуется использовать утилиту **[vlogscli](https://docs.victoriametrics.com/victorialogs/querying/vlogscli/)**.

Если искомое слово совпадает с ключевым словом LogsQL, его нужно заключить в кавычки (см. [документацию по строковым литералам](https://docs.victoriametrics.com/victorialogs/logsql/#string-literals)).
...
Обычно логи за прошлый год менее интересны, чем свежие. Поэтому рекомендуется добавлять к запросу **фильтр по времени** ([time filter](https://docs.victoriametrics.com/victorialogs/logsql/#time-filter)).
...
Если количество логов со словом `error` за последние 5 минут невелико (например, меньше нескольких миллионов), их можно отсортировать с помощью **конвейера `sort`** ([sort pipe](https://docs.victoriametrics.com/victorialogs/logsql/#sort-pipe)).
...
Скорее всего, вам не потребуется анализировать более нескольких сотен логов. Поэтому можно ограничить количество возвращаемых записей с помощью **конвейера `limit`** ([limit pipe](https://docs.victoriametrics.com/victorialogs/logsql/#limit-pipe)).
...
По умолчанию VictoriaLogs возвращает все **поля лога**. Если нужны только определённые поля, добавьте в конец запроса **конвейер `fields`** ([fields pipe](https://docs.victoriametrics.com/victorialogs/logsql/#fields-pipe)).
...
Имя поля можно заключить в кавычки, если оно содержит специальные символы или ключевые слова, конфликтующие с синтаксисом LogsQL. Любое слово также можно заключить в кавычки (см. [документацию по строковым литералам](https://docs.victoriametrics.com/victorialogs/logsql/#string-literals)).
...
Если поле `app` однозначно идентифицирует экземпляр приложения (т. е. для каждого уникального `app` работает один экземпляр), рекомендуется связать это поле с **полями потока логов** ([log stream fields](https://docs.victoriametrics.com/victorialogs/keyconcepts/#stream-fields)) при загрузке данных ([data ingestion](https://docs.victoriametrics.com/victorialogs/data-ingestion/)). Это обычно улучшает сжатие и производительность запросов при фильтрации по потокам с помощью фильтра `_stream` ([_stream filter](https://docs.victoriametrics.com/victorialogs/logsql/#stream-filter)).
...
LogsQL предоставляет **функции для расчёта статистики** ([stats pipe](https://docs.victoriametrics.com/victorialogs/logsql/#stats-pipe)) по выбранным логам.
