...
Конвейер `<q> | total_stats …` `конвейер` вычисляет **общие (глобальные) статистические показатели** (такие как `общее количество` или `общая сумма`) по указанным `полям логов`, возвращаемым запросом `<q>` `синтаксис запроса`, и сохраняет эти статистические данные в указанных полях лога для каждой входной записи.

Общая статистика вычисляется по логам, отсортированным по времени, поэтому запрос `<q>` **должен возвращать поле `_time`** `поле времени`, чтобы корректно рассчитать общую статистику.
...
Где:
- `stats_func*` — любая из поддерживаемых `функций общей статистики`;
...
- сумму значений поля `hits` — с помощью функции ``sum``:
...
Можно **опустить имя результата**. В этом случае имя результата будет равно строковому представлению использованной `функции общей статистики`.
...
Полезно комбинировать `total_stats` с `статистикой по временным интервалам`.
...
Это вычисляет `stats_func*` для каждой группы `(field1, ..., fieldM)` `полей лога`, встречающихся в логах, возвращённых запросом `<q>` `синтаксис запроса`.
