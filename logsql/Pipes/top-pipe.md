...
```  
(см. `документацию по конвейерам`) возвращает **топ‑`N`** наборов значений по полям `(field1, ..., fieldN)` (см. `поля логов`), которые встречаются в логах чаще всего — среди записей, отобранных запросом `<q>` (см. `синтаксис запросов`).

**Пример:** следующий запрос возвращает **топ‑7** `потоков логов` с наибольшим числом записей за последние 5 минут. Количество записей возвращается в поле `hits`:

```logsql
_time:5m | top 7 by (_stream)
```

Параметр `N` **необязателен**. Если он опущен, возвращается топ‑10 записей. Например, следующий запрос выдаёт топ‑10 значений поля `ip` (см. `модель данных`), встретившихся в логах за последние 5 минут:
