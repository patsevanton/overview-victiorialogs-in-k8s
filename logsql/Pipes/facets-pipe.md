...
Конвейер `<q> | facets` возвращает **наиболее частые значения** для каждого встреченного `поля лога`, полученного по запросу `<q>`. Также он выдаёт **оценку числа попаданий** (hits) для каждой пары `поле=значение`.
