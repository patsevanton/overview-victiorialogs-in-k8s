...
(`stats pipe function`) возвращает уникальные непустые значения по указанным `полям логов`.
...
(`field`) по логам за последние 5 минут:

```logsql
_time:5m | stats uniq_values(ip) unique_ips
```

Полученные уникальные IP-адреса можно развернуть в отдельные записи логов с помощью конвейера ``unroll``.
