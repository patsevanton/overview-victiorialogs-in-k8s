...
`max(field1, ..., fieldN)` возвращает максимальное значение среди всех указанных `полей логов`.
...
Функция ``row_max`` может использоваться для получения других полей, связанных с максимальным значением `duration`.
